...
# CART pozwalajšcym budowaæ drzewa klasyfikacyjne
install.packages("rpart.plot")
library(rpart.plot) #£adujemy pakiet pozwalaj¹cy tworzyæ wykresy drzew

# Pomoc R dotycz¹ca funkcji pakietów rpart i rpart.plot
help(package=rpart)
...
# ??cie¿ki prowadzšce do li??ci (wêz³ów koñcowych)
...
#Przewidywania drzewa optymalnego dotyczšce kategorii kredytu
predict(pdr,kr)
...
predict(dr1,ktr) # prognzowanie dla próby uczšcej
...
plot(iris[,3:4],col=iris[,5],pch=19,main="d³ugoœæ p³atka/szerokoœæ p³atka")

ir.tr <- tree(Species ~., iris)
ir.tr
(ir.tr1 <- snip.tree(ir.tr, nodes = c(12, 7)))
# Drzewo dla irysów, wykorzystuj¹ce dwie zmienne Petal.Length/Petal.Width (dwuwymiarowa przestrzeñ predyktorów)
plot(ir.tr1,main=)
text(ir.tr1)
summary(ir.tr1)
par(pty = "s")
plot(iris[,3:4],col=iris[,5],pch=19,main="d³ugoœæ p³atka/szerokosæ p³atka")
